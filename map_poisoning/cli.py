...
def geometry_slug(map_npy: str | None = None, scenario_preset: str | None = None) -> str:
    ...
    if map_npy:
        normalized = str(map_npy).replace("\\", "/")
...
def suggested_output_directory_from_args(args) -> str:
    ...
    return suggested_output_directory(
        method=getattr(args, "defense_method", "source_memory"),
...
def parser():
    ...
    p.add_argument("--seed",type=int,default=15); p.add_argument("--defense-method",choices=ALL_METHODS,default="source_memory")
...
def config_from_args(args):
    ...
    if args.scenario_preset and not map_npy and not args.map_movingai:
        from .scenario_presets import map_path_for_preset
        map_npy = map_path_for_preset(args.scenario_preset)
    ...
    if raw_methods:
        comparison_methods = tuple(item.strip() for item in str(raw_methods).split(",") if item.strip())
    elif bool(getattr(args, "compare", False)):
        comparison_methods = PRIMARY_METHODS
    else:
        comparison_methods = (args.defense_method,)
    return SimulationConfig(
        ...
        trust=TrustConfig(
            ...
            evidence_cap=float(args.trust_evidence_cap),
            confirmation_multiplier=float(args.trust_confirmation_multiplier),
