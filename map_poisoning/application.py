...
def run(config: SimulationConfig, *, comparison: bool = False, manifest_only: bool = False):
    ...
    if config.manifest_path:
        ...
        manifest=author_legacy_manifest(config, grid)
    else:
    ...
    # Kept for callers of the earlier modularization API; the two explicit
    # files above are the reproducibility record going forward.
    ...
    CsvMetrics.config(root/"run_metadata.json",{"python_version":sys.version,"platform":platform.platform(),"engine":config.engine,"settings_source":"original_legacy_cli","scenario_id":manifest.scenario_id,"manifest_hash":manifest.map_hash,"git_commit":commit,"git_dirty":dirty})
