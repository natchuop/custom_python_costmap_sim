...
from .models import AttackEvent, AttackType, ClaimType, DeliveryTask, ReportAuditLabel, TemporaryObstacleEpisode
...
def _is_valid_recon_attack_cell(cell, grid, goals, robots, heatmap):
    ...
    if any(
        ...
        return False
    report_cells = fake_report_cells(cell, 3, 3, grid)
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            if any(manhattan(cell, center) < MALICIOUS_FAKE_OBJECT_CENTER_MIN_SPACING for center in placed_centers):
                continue
            if not _is_valid_recon_attack_cell(cell, grid, goals, robots, heatmap):
                continue
            height, width = sample_fake_obstacle_dimensions(rng)
            report_cells = fake_report_cells(cell, height, width, grid)
...
def run_clean_recon_rollout(config: SimulationConfig, manifest: ScenarioManifest):
    ...
    _, robots, log = run_manifest_rollout(recon_config, manifest, "full_trust")
    heatmap = np.zeros(manifest.map_shape, dtype=np.int32)
    for sample in log["timeseries"]:
        if sample["step"] >= config.phases.recon_steps:
            continue
        if sample["robot_id"] in manifest.benign_robot_ids:
            position = sample["position"]
            heatmap[position] += 1
    recon_frame = min(config.phases.recon_steps - 1, config.phases.recon_steps)
    if recon_frame < 0:
        recon_frame = 0
...
def save_traffic_heatmap_artifacts(root: Path, heatmap: np.ndarray, *, title: str = "Reconnaissance traffic heatmap") -> None:
    ...
    try:
        ...
        image = axis.imshow(masked, origin="upper", cmap="hot")
...
def author_warehouse_manifest(config: SimulationConfig, grid=None) -> ScenarioManifest:
    ...
    candidates = recon_heatmap_attack_candidates(grid, goals, robots, heatmap, rng=place_rng)
    if not candidates:
        candidates = recon_heatmap_attack_candidates(
            grid, goals, robots, heatmap, rng=place_rng, require_route_overlap=False
        )
    if not candidates:
